...
const Card = ({ product }) => {
  const { name, category, quantity, price, images, _id, description } = product;
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/4] w-full overflow-hidden rounded-xl bg-gray-100 mb-4">
        ...
        >
          <div className="bg-blue-500 text-white p-3 rounded-full shadow-xl">
            <FaEye size={18} />
          </div>
        </Link>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-2">
        ...
        <div className="flex justify-between items-center mt-2 text-sm text-gray-600">
          ...
          <p className="text-blue-600 font-extrabold text-lg">${price}</p>
        </div>
      </div>
      ...
      <div className="mt-4">
        <Link
          to={`/products/${_id}`}
          className="w-full flex items-center justify-center gap-2 py-3 bg-blue-600 text-white/70 font-bold rounded-xl hover:bg-blue-500 hover:text-white transition-all duration-300"
        >
          View Details
        </Link>
      </div>
