          ...
          <div className="hidden md:flex items-center gap-8">
            {currentLinks.map(link => (
              <NavLink
                ...
                to={link.path}
                className={({ isActive }) =>
                  `text-sm font-bold  transition-colors hover:text-blue-500 ${isActive ? 'text-blue-500 border-b-2 border-blue-500' : ''
                  }`
                }
              >
                {link.name}
          ...
          <div className="flex items-center gap-4">
            ...
            {!user && (
              <div className="hidden md:flex items-center gap-2">
                <Link
                  to="/login"
                  className="text-sm font-bold px-4 py-1.5 rounded-lg border border-blue-500 text-blue-600 hover:bg-blue-50 transition"
                >
                  Login
                </Link>
                <Link
                  to="/signup"
                  className="text-sm font-bold px-4 py-1.5 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition shadow"
                >
                  Register
                </Link>
              </div>
            ...
            {/* Profile Dropdown */}
            <div className={`relative ${!user ? 'md:hidden' : ''}`}>
              <div
              ...
              {isOpen && (
                <div className={`absolute right-0 top-12 w-50  rounded-b-xl shadow-xl  animate-in fade-in zoom-in text-center duration-200 ${theme === 'dark' ? 'bg-gray-900/95 backdrop-blur-md text-white' : 'bg-white backdrop-blur-md text-gray-800'
                  ...
                  <div className="md:hidden pt-2">
                    {currentLinks.map(link => (
                      <Link
                        ...
                        className="block px-4  font-bold py-2 tex text-sm hover:text-blue-500"
                      >
                        {link.name}
                      </Link>
                    ))}
                  ...
                  {user ? (
                    <>
                      <Link to="/dashboard" onClick={() => setIsOpen(false)} className="block px-4 font-bold py-2 tex text-sm hover:text-blue-500">Dashboard</Link>
                      <Link to="/dashboard/profile" onClick={() => setIsOpen(false)} className="block px-4  font-bold py-2 tex text-sm hover:text-blue-500"> Profile</Link>
                    ...
                    <>
                      <Link to="/login" onClick={() => setIsOpen(false)} className="block px-4 font-bold py-2 tex text-sm hover:text-blue-500">Login</Link>
                      <Link to="/signup" onClick={() => setIsOpen(false)} className="block px-4  font-bold py-2 tex text-sm hover:text-blue-500">Sign Up</Link>
                    </>
                  )}
                </div>
              )}
