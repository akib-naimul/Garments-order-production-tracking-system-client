...
const AddProductForm = ({ onSuccess }) => {
  ...
  const onSubmit = async (data) => {
    if (userData?.status === 'Suspended') {
      toast.error("Your account is suspended. You cannot add new products.");
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white p-6 md:p-10 rounded-2xl shadow-xl border border-gray-100">
      ...
      <div className="flex items-center gap-3 mb-8 border-b pb-4">
        <div>
          <h2 className="text-3xl font-black text-blue-600 tracking-tight">Add New Product</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="flex flex-col gap-1">
            <label className="text-sm font-bold text-gray-600 flex items-center gap-1 ml-1">
              <TbPhotoPlus className="text-blue-600" /> Images * (Comma separated URLs)
            </label>
            <input
        ...
        {userData?.status === "Suspended" ? (
          ...
          <button
            ...
            className={`w-full font-black py-4 rounded-xl transition-all duration-300 shadow-lg active:scale-[0.98] flex items-center justify-center gap-2
              ${loading ? 'bg-gray-400 cursor-not-allowed text-white' : 'bg-blue-400 hover:bg-blue-500 text-white'}`}
          >
