...
const ProductDetails = () => {
  ...
  return (
    <div className="mt-6 overflow-x-hidden">
      <Container>
        <nav className="py-6">
          <Link to="/products" className="group flex items-center gap-2 text-gray-400 hover:text-blue-600 transition-all font-semibold text-sm">
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                {images.map((img, i) => (
                  <button
                    ...
                    className={`relative w-14 h-16 sm:w-16 sm:h-20 rounded-2xl overflow-hidden border-2 transition-all ${activeImage === i ? "border-blue-500 scale-105" : "border-transparent opacity-60 hover:opacity-100"}`}
                  >
          ...
          <div className="space-y-8">
            <header className="space-y-4">
              ...
              <div className="flex flex-wrap items-center gap-6 text-sm font-bold text-gray-400">
                <span className="flex items-center gap-1.5"><HiOutlineCube className="text-blue-500" /> ID: {product._id.toUpperCase()}</span>
                <span className="flex items-center gap-1.5"><MdOutlineProductionQuantityLimits className="text-blue-500" /> MOQ: {product.minimumOrder} Pcs</span>
                <span className="flex items-center gap-1.5"><HiStar className="text-yellow-400" /> 4.8 (24 Reviews)</span>
              </div>
            </header>

            {/* Price Card */}
            <div className=" rounded-3xl p-8 flex items-center justify-between border border-blue-100/50 shadow-sm">
              <div>
                <p className="text-blue-700 font-black text-xs uppercase tracking-[0.2em] mb-1 text-nowrap">Wholesale Price</p>
                <h2 className="text-5xl font-black text-blue-600 tracking-tighter">${product.price}</h2>
              </div>
              <div className="text-right hidden sm:block border-l-2 border-blue-200 pl-8">
            ...
            <div className="space-y-4">
              {product.quantity === 0 ? (
                ...
                <Button
                  ...
                  className="w-full py-5 shadow-2xl shadow-blue-500/20"
                />
              ) : (
                <div className=" p-6 rounded-xl border border-gray-500/50">
                  <p className=" text-xs font-bold text-center leading-relaxed">
                    Wholesale pricing & booking is restricted to <span className="text-blue-400">Verified Buyers</span> only.
                  </p>
                  <Link to="/login" className="block text-center mt-3 text-blue-400 font-black uppercase text-xs hover:underline tracking-widest">Login as Buyer</Link>
                </div>
              )}
            </div>
                    ...
                    <motion.div key="sp" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="grid md:grid-cols-2 gap-4">
                      {product.features?.map((f, i) => (
                        <div key={i} className="flex items-center gap-3 text-sm text-gray-600 font-bold  p-4 rounded-2xl border border-gray-500/50 shadow-sm">
                          <HiOutlineBadgeCheck className="text-blue-500" size={22} /> {f}
                        </div>
                      ))}
                    </motion.div>
                  )}
        ...
        {related.length > 0 && (
          <div className="mt-5">
            <div className="flex justify-between items-end mb-8">
              <div>
                <h2 className="text-3xl font-black">Related <span className="text-blue-500">Items</span></h2>
                <p className="text-gray-400 font-bold text-sm">You might also be interested in these products</p>
              </div>
              <Link to="/products" className="text-blue-600 font-black text-xs uppercase border-b-2 border-blue-500 pb-1">View All</Link>
            </div>
