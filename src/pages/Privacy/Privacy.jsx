        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12 border-b border-gray-500/40 pb-8">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            <h1 className="text-4xl md:text-5xl font-black  pb-6">
              {content[lang].title} <span className="text-blue-500">{content[lang].subtitle}</span>
            </h1>
          ...
          <div className="flex  flex-wrap gap-3">
            <button onClick={handlePrint} className="p-2.5 border border-gray-500/40 rounded-xl hover:border-blue-500 transition shadow-sm group">
              <FiPrinter className="group-hover:text-blue-500" />
            </button>
            <button onClick={handleDownload} className="p-2.5 border border-gray-500/50  rounded-xl hover:border-blue-500 transition shadow-sm group">
              <FiDownload className="group-hover:text-blue-500" />
            </button>
          </div>
        </div>
          ...
          <div className="md:col-span-4 space-y-3 hidden md:block">
            ...
            {content[lang].sections.map((sec) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition-all ${activeAccordion === sec.id ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 border-l-4 border-blue-500 shadow-sm' : 'hover:text-white/70  hover:bg-gray-50 dark:hover:bg-blue-800'}`}
              >
          ...
          <div className="md:col-span-8 space-y-4">
            {content[lang].sections.map((sec, index) => (
              <motion.div
                ...
                className={`border border-gray-500/20 rounded-2xl overflow-hidden transition-all ${activeAccordion === sec.id ? 'ring-2 ring-blue-500/20 border-blue-200' : ''}`}
              >
                <button
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={`p-2 rounded-lg ${activeAccordion === sec.id ? 'bg-blue-500 text-white' : ' text-gray-500'}`}>
                      {sec.icon}
                    </div>
                    <span className="text-xl font-bold ">{sec.title}</span>
                  </div>
                  <FiChevronDown className={`transition-transform duration-300 ${activeAccordion === sec.id ? 'rotate-180 text-blue-500' : 'text-gray-400'}`} />
                </button>
